...
import {
  ...
  Divider, FormControlLabel, IconButton, MenuItem, Stack, Switch, Table, TableBody,
...
function periodsOverlap(a: SchedulePeriod | SchedulePeriodInput, b: SchedulePeriod): boolean {
...
export default function WorkingHoursPage() {
  ...
  const [periodDraft, setPeriodDraft] = useState<SchedulePeriodInput | null>(null);
  ...
  const periods = useMemo(
    () => [...(periodsQuery.data ?? [])].sort((a, b) => a.validFrom.localeCompare(b.validFrom)),
    [periodsQuery.data],
  );
  ...
  const hoursQuery = useQuery({
    ...
    queryFn: () => workingHoursApi.listWorkingHours(activeCalendarId, activePeriodId),
  ...
  const deletePeriod = useMutation({
    mutationFn: (id: string) => workingHoursApi.deletePeriod(activeCalendarId, id),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['periods', activeCalendarId] });
      setPeriodId('');
    },
  ...
  const checkImpactThenSave = useMutation({
    mutationFn: async (input: SchedulePeriodInput): Promise<ImpactReport | null> => {
  ...
  const periodIsValid =
    ...
    (periodDraft.validTo === null || periodDraft.validTo >= periodDraft.validFrom) &&
    !overlapping;
    ...
    <Box sx={{ maxWidth: 1200, mx: 'auto' }}>
      ...
      >
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 1 }}>
          <Typography variant="h6">{t('booking.workingHours.periods')}</Typography>
          <Button
            size="small"
            startIcon={<AddIcon />}
            onClick={() => {
        ...
        {activePeriod ? (
          <>
            ...
            <DayActivityGrid
              ...
              workingDays={
                new Set(
                  (hoursQuery.data ?? []).filter((h) => h.isActive).map((h) => h.dayOfWeek),
                )
              }
            />
          </>
          ...
          {periodDraft ? (
            <Stack spacing={2} sx={{ mt: 1 }}>
              ...
              <TextField
                ...
                onChange={(e) => setPeriodDraft({ ...periodDraft, validFrom: e.target.value })}
              ...
              <TextField
                ...
                onChange={(e) =>
                  setPeriodDraft({ ...periodDraft, validTo: e.target.value || null })
                }
                helperText={t('booking.workingHours.validToHelp')}
                slotProps={{ inputLabel: { shrink: true } }}
              />
              {overlapping ? (
                <Alert severity="error">{t('booking.workingHours.overlap')}</Alert>
              ) : null}
              {savePeriod.error ? (
                <Alert severity="error">{errorText(savePeriod.error, t)}</Alert>
              ) : null}
              {checkImpactThenSave.error ? (
                <Alert severity="error">{errorText(checkImpactThenSave.error, t)}</Alert>
              ) : null}
            </Stack>
          ) : null}
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setPeriodDraft(null)}>{t('booking.common.cancel')}</Button>
          <Button
            variant="contained"
            disabled={!periodIsValid || savePeriod.isPending || checkImpactThenSave.isPending}
      ...
      <Dialog open={impact !== null} onClose={() => setImpact(null)} fullWidth maxWidth="md">
        ...
        <DialogActions>
          <Button onClick={() => setImpact(null)}>{t('booking.common.cancel')}</Button>
          <Button
            color="warning"
            variant="contained"
...
function DayRow({ dayOfWeek, period, calendarId, today, existing, workers }: DayRowProps) {
  ...
  const invalidate = () =>
    queryClient.invalidateQueries({ queryKey: ['working-hours', calendarId, period.id] });
    ...
    <TableRow hover>
        ...
        <FormControlLabel
          control={
            <Switch
              ...
              onChange={(e) => setEdited({ ...value, isActive: e.target.checked })}
            />
          }
      ...
      <TableCell sx={{ verticalAlign: 'top', pt: 2 }}>
        <Stack direction="row" spacing={1}>
          <TextField
            ...
            disabled={!value.isActive}
            onChange={(e) => setEdited({ ...value, breakStart: e.target.value || null })}
            slotProps={{ inputLabel: { shrink: true } }}
            sx={{ width: 120 }}
          />
          <TextField
            ...
            disabled={!value.isActive}
            onChange={(e) => setEdited({ ...value, breakEnd: e.target.value || null })}
            slotProps={{ inputLabel: { shrink: true } }}
            sx={{ width: 120 }}
          />
        </Stack>
      ...
      <TableCell sx={{ verticalAlign: 'top', pt: 2, minWidth: 240 }}>
        <TextField
          ...
          onChange={(e) =>
            setEdited({
              ...value,
              ...modeToCycle(e.target.value as CycleMode, value.repeatEveryNWeeks),
            })
          }
        >
          <MenuItem value="every">{t('booking.workingHours.cycleEvery')}</MenuItem>
          <MenuItem value="even">{t('booking.workingHours.cycleEven')}</MenuItem>
          <MenuItem value="odd">{t('booking.workingHours.cycleOdd')}</MenuItem>
          <MenuItem value="everyNth">{t('booking.workingHours.cycleNth')}</MenuItem>
        </TextField>
        {mode === 'everyNth' ? (
          <TextField
            type="number"
            ...
            onChange={(e) =>
              setEdited({ ...value, repeatEveryNWeeks: Math.max(2, Number(e.target.value) || 2) })
            }
          />
        ) : null}
        {value.isActive ? (
      ...
      <TableCell sx={{ verticalAlign: 'top', pt: 2, minWidth: 180 }}>
        <TextField
          ...
          disabled={!value.isActive}
          onChange={(e) => setEdited({ ...value, workerUserId: e.target.value || null })}
        >
