...
function kindOf(exception: { isClosed: boolean; workerUserId: string | null }): ExceptionKind {
...
export default function ExceptionsPage() {
  ...
  const exceptions = useMemo(
    () => [...(exceptionsQuery.data ?? [])].sort((a, b) => a.date.localeCompare(b.date)),
  ...
  const save = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['exceptions', activeCalendarId] });
      setDraft(null);
    },
  });

  const remove = useMutation({
    mutationFn: (id: string) => workingHoursApi.deleteException(activeCalendarId, id),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['exceptions', activeCalendarId] }),
  });
    ...
    <Box sx={{ maxWidth: 1000, mx: 'auto' }}>
      ...
      >
        ...
        <AsyncSection
          ...
          emptyAction={{ label: t('booking.exceptions.new'), onClick: openCreate }}
                ...
                {exceptions.map((exception) => (
                  <TableRow key={exception.id} hover>
                    ...
                    <TableCell>
                      ...
                      <Chip
                        size="small"
                        label={t(`booking.exceptions.kind.${kindOf(exception)}`)}
                      ...
                      {exception.isClosed
                        ...
                        : [
                            exception.startTime && exception.endTime
                              ...
                              : null,
                            workers.find((w) => w.id === exception.workerUserId)?.name ?? null,
                          ]
                            .filter(Boolean)
                            .join(' · ') || '-'}
                    </TableCell>
        ...
        <DialogContent>
          {draft ? (
            <Stack spacing={2} sx={{ mt: 1 }}>
              ...
              >
                <MenuItem value="closed">{t('booking.exceptions.kind.closed')}</MenuItem>
              ...
              {kind === 'differentWorker' ? (
                <TextField
                  ...
                  onChange={(e) => setDraft({ ...draft, workerUserId: e.target.value || null })}
                >
                  {workers.map((worker) => (
